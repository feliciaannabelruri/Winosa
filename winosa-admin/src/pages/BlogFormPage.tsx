...
const BlogFormPage: React.FC = () => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append('image', file); // field name harus 'image' sesuai multer
      const res = await api.post('admin/upload?folder=blog', fd, {
        headers: { 'Content-Type': undefined }, // biar axios set boundary otomatis
      });
  ...
  const validate = (isPublished: boolean): boolean => {
    if (!form.title.trim()) { toast.error('Title wajib diisi'); return false; }
    if (!form.slug.trim()) { toast.error('Slug wajib diisi'); return false; }
    if (!form.author.trim()) { toast.error('Author wajib diisi'); return false; }
    if (!form.content.trim() || form.content === '<p></p>' || form.content === '<p><br></p>') {
      toast.error('Content wajib diisi'); return false;
    }
    if (isPublished && !imageUrl) {
      toast.error('Featured image wajib diisi sebelum publish'); return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (isPublished: boolean) => {
    ...
    // Build FormData — backend multer expects multipart/form-data
    // NOTE: image sudah diupload terpisah via handleImageUpload, di sini hanya kirim URL-nya
    const fd = new FormData();
    ...
    fd.append('tags',        JSON.stringify(allTags));   // array → JSON string
    fd.append('isPublished', String(isPublished));
    if (imageUrl) fd.append('image', imageUrl);          // URL string (sudah diupload)
