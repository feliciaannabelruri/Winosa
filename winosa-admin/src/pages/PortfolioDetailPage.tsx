...
const PortfolioDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    portfolioService
      ...
      .catch(() => {
        toast.error(t('portfolio_not_found'));
        navigate('/portfolio');
      })
    ...
    return (
      <div className="space-y-5 animate-pulse">
        ...
        {[...Array(4)].map((_, i) => (
          <div
            key={i}
            className="w-full h-14 bg-gray-100 rounded-2xl"
          />
        ))}
      </div>
    );
    ...
    <div className="space-y-6 w-full overflow-x-hidden">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <span
            className={`inline-block text-xs font-semibold px-3 py-1 rounded-full border mb-2 ${
              portfolio.isActive
          ...
          <h1 className="text-3xl font-display font-bold text-dark mt-2">
            {portfolio.title}
          </h1>
        ...
        <button
          onClick={() =>
            navigate(`/portfolio/edit/${portfolio._id}`)
          }
      ...
      {/* Long Description */}
      {portfolio.longDesc && (
        <SectionCard title={t('long_description')}>
      ...
      {/* Project Info */}
      <SectionCard title={t('project_information')}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {portfolio.techStack.map((tech, i) => (
                  <span
                    key={i}
                    className="px-3 py-1.5 bg-gray-100 text-dark text-xs font-semibold rounded-full"
                  >
                    {tech}
                  </span>
                ))}
      ...
      {/* Case Study */}
      <SectionCard title={t('case_study')}>
      ...
      {portfolio.metrics &&
        portfolio.metrics.length > 0 && (
          <SectionCard title={t('key_metrics')}>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {portfolio.metrics.map((m, i) => (
                <div
                  key={i}
                  className="bg-gray-50 rounded-2xl px-4 py-5 text-center"
                >
      ...
      {portfolio.gallery &&
        portfolio.gallery.length > 0 && (
          <SectionCard title={t('project_gallery')}>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {portfolio.gallery.map((url, i) => (
                <div
                  key={i}
                  className="aspect-video rounded-2xl overflow-hidden border border-gray-100"
                >
                  <img
                    src={url}
                    alt={`gallery-${i}`}
      ...
      {portfolio.projectUrl && (
        <SectionCard title={t('project_url')}>
          <a
