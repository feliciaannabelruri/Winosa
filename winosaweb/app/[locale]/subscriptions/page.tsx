...
const FEATURES_COMPARE = [
  { label: { en: "Content updates", id: "Pembaruan konten", nl: "Content updates" }, starter: "2x/month", business: "8x/month", enterprise: "Unlimited" },
  { label: { en: "Backup", id: "Pencadangan (Backup)", nl: "Backup" }, starter: "Weekly", business: "Daily", enterprise: "Real-time" },
  { label: { en: "Uptime monitoring", id: "Pemantauan Uptime", nl: "Uptime monitoring" }, starter: true, business: "✔ + alerting", enterprise: "✔ SLA 99.5%" },
  { label: { en: "Support response", id: "Respons dukungan", nl: "Support response" }, starter: "1-2 days", business: "< 8 hours", enterprise: "< 2 hours" },
  { label: { en: "SSL & security", id: "SSL & Keamanan", nl: "SSL & security" }, starter: "Basic", business: true, enterprise: true },
  { label: { en: "SEO Optimization", id: "Optimasi SEO", nl: "SEO teknis" }, starter: false, business: true, enterprise: true },
  { label: { en: "Monthly report", id: "Laporan bulanan", nl: "Monthly report" }, starter: true, business: "✔ + analytics", enterprise: "✔ + strategy" },
  { label: { en: "Dedicated manager", id: "Manajer khusus", nl: "Dedicated manager" }, starter: false, business: false, enterprise: true },
  { label: { en: "Bug fix", id: "Perbaikan Bug", nl: "Bug fix" }, starter: false, business: "Minor", enterprise: "Included" },
  { label: { en: "Technical consultation", id: "Konsultasi Teknis", nl: "Technical consultation" }, starter: false, business: false, enterprise: "Monthly" },
];
...
const PLAN_ICON: Record<string, React.ReactNode> = {
  ...
  scale: <Shield className="w-6 h-6" />,
};
...
export default function SubscriptionsPage() {
    ...
    const run = async () => {
      const translated = await Promise.all(
        plans.map(async (p) => ({
          ...
          billingCycle: p.billingCycle ? await translateHybrid(p.billingCycle, language, tApi) : undefined
        }))
      );
  ...
  const getScheme = (name: string, isPopular?: boolean) => {
    ...
    if (isPopular || n.includes("pro") || n.includes("business") || n.includes("growth"))
    ...
    if (n.includes("enterprise") || n.includes("scale"))
  ...
  return (
    <main aria-label="Subscription plans page" className="w-full bg-white overflow-x-hidden">
      ...
      {/* ── HERO ── */}
      <section
        className="relative w-full min-h-[60vh] flex flex-col items-center justify-center text-center px-6 overflow-hidden"
        ...
        <div className="max-w-7xl mx-auto px-6">
                    ...
                    <div className={`relative rounded-[32px] p-8 border-2 ${scheme.border} ${scheme.bg} ${scheme.text} flex flex-col h-full transition-transform duration-300 hover:-translate-y-2 hover:shadow-2xl`}>
                      ...
                      <div className="mb-8">
                        <p className={`text-xs uppercase tracking-widest mb-1 ${plan.isPopular ? "text-white/40" : "text-black/30"}`}>
                          {t("pricingPage", "startingFrom")}
                        </p>
                        <p className="text-3xl font-bold">
                          {plan.price
                            ? `${plan.currency || "Rp"} ${Number(plan.price).toLocaleString("id-ID")}`
                            : "Contact us"}
                        </p>
                        {plan.billingCycle && (
                      ...
                      >
                        {plan.cta || plan.buttonLabel || `${t("pricingPage", "getStarted")} ${plan.name}`}
          ...
          <FadeUp delay={0.3}>
            <div className="mt-12 text-center">
              ...
              >
                {t("pricingPage", "requestCustom")} <ArrowRight className="w-4 h-4" aria-hidden="true" />
              </Link>
            </div>
          </FadeUp>
        </div>
        ...
        <div className="max-w-5xl mx-auto px-6">
          <FadeUp>
            <div className="text-center mb-12">
              ...
              >
                {showComparison ? t("pricingPage", "hideComparison") : t("pricingPage", "showComparison")}
                ...
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b-2 border-black/10">
                      <th className="text-left py-4 px-4 text-sm font-semibold text-black/40 uppercase tracking-wide w-1/4">{t("pricingPage", "featureHeader") || "Feature"}</th>
                      <th className="text-center py-4 px-4 text-sm font-bold text-black">Starter</th>
                      <th className="text-center py-4 px-4 text-sm font-bold text-black">Growth</th>
                      <th className="text-center py-4 px-4 text-sm font-bold text-black">Scale</th>
                    </tr>
                  </thead>
                  <tbody>
                    {FEATURES_COMPARE.map((row, i) => (
                      ...
                      >
                        ...
                        {(["starter", "business", "enterprise"] as const).map((tier) => (
                          <td key={tier} className="py-4 px-4 text-center text-sm font-medium text-black/60">
                            {row[tier] === true ? (
        ...
        <div className="max-w-3xl mx-auto px-6">
          ...
          <FadeUp delay={0.3}>
            <div className="mt-12 text-center p-8 rounded-[24px] bg-[#f8f7f5] border border-black/10">
              <p className="text-black font-semibold mb-2">{t("pricingPage", "stillHaveQuestions")}</p>
              <p className="text-black/50 text-sm mb-6">
                {t("pricingPage", "faqSupport")}
              </p>
              <Link
                href={localePath("/Contact")}
