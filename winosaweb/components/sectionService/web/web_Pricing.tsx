...
export default function SectionPricingMobile({ data }: { data?: any }) {
  ...
  return (
    <>
      {/* ── Smart Recommend ── */}
      <SmartRecommend serviceType="mobile" onRecommend={handleRecommend} />
      ...
      <section className="w-full bg-white pb-32" ref={pricingRef}>
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {plans.map((plan, i) => {
            ...
            const whatsappLink  = `https://wa.me/6281234567890?text=Hello%20I%20am%20interested%20in%20${encodeURIComponent(plan.name)}`;
            ...
            return (
              <FadeUp key={i} delay={i * 0.2}>
                <div
                  ...
                  className="relative cursor-pointer rounded-[28px] p-10 bg-white transition-all duration-500 flex flex-col h-full"
                  style={{
                    boxShadow: isRecommended
                      ? "0 0 0 2.5px black, 0 0 80px rgba(196,168,50,0.55)"
                      : active === i
                      ? "0 0 90px rgba(255,200,80,0.8)"
                      : "none",
                    transform: isRecommended ? "scale(1.05)" : active === i ? "scale(1.02)" : "scale(1)",
